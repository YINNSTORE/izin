    ...
    async def perpanjang_(event):
        ...
        async with bot.conversation(chat) as exp:
            await event.respond("**Pilih Domain Nya**", buttons=[
                [Button.inline(" vpn-group.my.id ", "1"),
                 Button.inline(" yinn.my.id ", "2")],
                [Button.inline(" yiin.my.id ", "3"),
                 Button.inline(" vpn-premium.biz.id ", "4")]
            ])
            exp = exp.wait_event(events.CallbackQuery)
            exp = (await exp).data.decode("ascii")
        cmd = f'printf "%s\n" "{user}" "{pw}" "{exp}" | bash /root/regis/shell/domain'
        try:
